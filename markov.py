...
def valueof(action,s,V,gamma,mdp): # What is the value of action in s w.r.t. V?
  ...
  succ = mdp.successors(s,action)
  value=0

  for x in succ:
  	value+=x[1]*(x[2]+gamma*(V.get(x[0],0)))
  return value
  	...
  	for s in range(0,mdp.stateMax+1):
	  	...
	  	while(1):
	  		...
	  		for a in mdp.applicableActions(s):
	  			val = valueof(a,s,V,gamma,mdp)
	  			if maxi < val:
	  				maxi=val
	  		V[s]=maxi
	  		if abs(V.get(s,0)-v_first) < 0.01*((1-gamma)/(2*gamma)):
	  			break
	  		n=n+1
  return V
...
def bestActionFor(s,gamma,mdp,V):
  ...
  for i in range(0,150):
  	for s1 in range(0,mdp.stateMax+1):
	  	n=0
	  	maxi=0
	    
	  	while(1):
	  		v_first = A.get(s1,0)
	  		for a in mdp.applicableActions(s1):
	  			val = valueof(a,s1,A,gamma,mdp)
	  			if maxi < val:
	  				if s1==s:
	  					bestAction = a
	  				maxi=val
	  		A[s1]=maxi
	  		if abs(A.get(s1,0)-v_first) < 0.01*((1-gamma)/(2*gamma)):
	  			break
	  		n=n+1
